...
import { IEditCard, IResponseCollection } from "@internal/shared"
...
const CardSidebar = () => {
  ...
  const [lastResponseCard, setLastResponseCard] = useState<IEditCard | null>(
    null
  )
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const { status, message, data } = await CardsServices.getSingleCard(
        cardId
      )
  ...
  const handleChangeCollectionId = (
          ...
          <>
            ...
            <Autocomplete
              ...
              options={collections.filter(
                (c) => c.createdBy === lastResponseCard?.createdBy
              )}

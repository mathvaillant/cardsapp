...
const Cards = () => {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const cardStateFromEvent = cards.find((card) =>
        _.keys(data_changed.cards).includes(card._id)
      )

      if (reason === "card_added" || !cardStateFromEvent) {
        const cardKeyFromEvent =
          _.keys(data_changed.cards).find(
            (key) => data_changed.cards[key] === "child_added"
          ) || ""
      ...
      if (reason === "card_updated") {
        const { data } = await CardsServices.getSingleCard(
          cardStateFromEvent?._id
        )
        if (!data) return
  ...
  return (
    <>
      <div className="Cards">
        ...
        <NewCardModal modalOpen={modalOpen} handleClose={handleToggleModal} />
      </div>
      <Outlet />
    </>
  )

...
const Collections = () => {
  ...
  const [collections, setCollections] = useState<IResponseCollection[]>([])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const ColStateFromEvent = collections.find((collection) =>
        _.keys(data_changed.collections).includes(collection._id)
      )

      if (reason === "collection_added" || !ColStateFromEvent) {
        const colKeyFromEvent =
          _.keys(data_changed.collections).find(
            (key) => data_changed.collections[key] === "child_added"
          ) || ""

        const { data } = await CollectionServices.getSingleCollection(
          colKeyFromEvent
        )
        if (!data) return
      ...
      if (reason === "collection_deleted") {
        setCollections(
          collections.filter(
            (collection) => collection._id !== ColStateFromEvent?._id
          )
        )
        return
      }
      ...
      if (reason === "collection_updated") {
        const { data } = await CollectionServices.getSingleCollection(
          ColStateFromEvent._id
        )
        if (!data) return

        console.log([data, ...collections])
        const collectionsUpdated = _.uniq(
          [data, ...collections],
          false,
          _.iteratee("_id")
        )
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const { status, message, data, totalPages } =
  ...
  useDebounceCallback(() => setDebouncedSearchValue(searchValue), 750, [
    searchValue
  ])
    ...
    <>
      <div className="Collections">
        ...
        >
          ...
          {!!collections.length && (
            <Paginate totalPages={totalPages} onChange={handleChangePage} />
          )}
        </Box>
      </div>
      <NewCollectionModal

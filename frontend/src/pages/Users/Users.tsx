...
const Users = () => {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const userStateFromEvent = users.find((user) =>
        _.keys(data_changed.users).includes(user._id)
      )

      if (reason === "user_added" || !userStateFromEvent) {
        const userKeyFromEvent =
          _.keys(data_changed.users).find(
            (key) => data_changed.users[key] === "child_added"
          ) || ""
